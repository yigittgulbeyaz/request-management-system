...
erDiagram
    ...
    USERS {
        ...
        number is_locked
        varchar2 setup_token
        timestamp setup_token_expires_at
...
2. Tables
   ...
   `password_hash`	`VARCHAR2(255)`	Nullable	BCrypt output; null until the account is set up
   ...
   `security_question`	`VARCHAR2(100)`	Nullable	Enum name, not the question text; chosen during setup
   `security_answer_hash`	`VARCHAR2(255)`	Nullable	Hashed with the same encoder as passwords
   ...
   `setup_token`	`VARCHAR2(64)`	Nullable	One-time code; cleared the moment it is used
   `setup_token_expires_at`	`TIMESTAMP`	Nullable	Set alongside the token
   `preferred_theme`	`VARCHAR2(10)`	default `'light'`
   `preferred_language`	`VARCHAR2(5)`	default `'tr'`
   `created_at`	`TIMESTAMP`	`NOT NULL`, default `SYSTIMESTAMP`
   An account has a code or a password, never both. A `CHECK` enforces it:
```sql
CHECK ((setup_token IS NOT NULL AND password_hash IS NULL)
    OR (setup_token IS NULL AND password_hash IS NOT NULL))
```
An administrator opens an account without credentials and hands over a one-time code; the person who uses it chooses the password and the security question, and the code is destroyed in the same transaction. That is why three columns that look like they should be mandatory are nullable: an account waiting to be set up is a real state, and the schema has to be able to say so.
It is also why an administrator never learns a lasting password for someone else. One who chose the password would hold the account open indefinitely, and one who also chose the security question would hold both doors at once.
`is_active` and `is_locked` are distinct states. Inactive means an administrator disabled the account; locked means the self-service recovery attempt limit was exceeded. They have different causes, different remedies, and different UI treatment, so collapsing them into one column would lose information. Unlocking does not reactivate, and reactivating does not unlock.
...
`deadline`	`TIMESTAMP`	Nullable	Committed at conversion, derived from the priority band
...
`deadline` is stored rather than derived. The priority band answers how long the work gets — two days for critical, five for high, ten for medium, twenty for low — and the deadline follows from it at conversion. Recomputing it on read would let a change to the bands quietly rewrite commitments already made, and a promise that moves is not a promise.
It is set when the request is scheduled rather than when a developer claims it. A developer inherits the deadline by taking the task; starting a fresh clock on assignment would let work sit unclaimed for a month and still arrive on time.
Nullable because tasks converted before the rule existed have none, and because saying so is better than showing a date nobody promised.
2.5 `NOTIFICATIONS`
Column	Type	Constraints	Notes
...
4. Indexes
   ...
   `IDX_HISTORY_REQUEST`	`request_status_history(request_id, changed_at DESC)`	Audit trail lookup per request, newest first
   `IDX_HISTORY_NEW_STATUS`	`request_status_history(new_status)`	Test rework rate scans transitions by target status
